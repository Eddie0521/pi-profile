...
你想让 Agent 在代码审查时的表现，跟它在排查生产事故时的表现不一样；你也很难想象，在做新领域调研时它该跟前面两种情况一样。一个 Pi 会话不该同时把这几件事都干了。

配置文件（profile）做的就是这件事：把系统提示词、模型绑定、技能和工具权限打包成一个可切换的角色。定义一次，任务切换时换人。安全审计的只读审核员、断网写作模式、规划者+执行者的开发团队：每个都是一份 JSON。
